...
contract StableCoin is ERC20 {
    ...
    function depositCollateralBuffer() external payable {
        ...
        if (deficitOrSurplusInUsd <= 0) {
            ...
            if(msg.value < deficitInEth + requiredInitialSurplusInEth) {
                ...
                revert InitialCollateralRatioError("STC: Initial collateral ratio not met, minimus is ", minimumDepositAmount);
            }
        ...
        WadLib.Wad dpcInUsdPrice = _getDPDinUsdPrice(surplusInUsd);
        uint256 mintDepositorCoinAmount = (msg.value.mulWad(dpcInUsdPrice) / oracle.getPrice()) ;

        depositorCoin.mint(msg.sender, mintDepositorCoinAmount);
    }

    function withdrawCollateralBuffer(uint256 burnDpositorCoinAmount) external {
        require(depositorCoin.balanceOf(msg.sender) >= burnDpositorCoinAmount, 
        "STC: Sender has insuffient DPC funds");

        depositorCoin.burn(msg.sender, burnDpositorCoinAmount);
        ...
        uint256 surplusInUsd = uint256(deficitOrSurplusInUsd);
        WadLib.Wad dpcInUsdPrice = _getDPDinUsdPrice(surplusInUsd);
        uint256 refundingUsd = burnDpositorCoinAmount.mulWad(dpcInUsdPrice);
    ...
    function _getDPDinUsdPrice(uint256 surplusInUsd) private view returns (WadLib.Wad) {
